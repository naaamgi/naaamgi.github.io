...
| 항목 | 내용 |
|------|------|
| 기간 | 2026.02.23 ~ 2026.04.12 (공휴일 제외 31일) |
| 팀 | 4조, 11명 |
| 멘토 | 장OO |
| 역할 | 인프라(AWS, VPN, 로컬 내부망) 구축 및 인프라 시나리오 설계 및 수행, 보고서, PPT, 회의록 작성 |

의도적으로 취약한 대학병원 웹/모바일 서비스를 개발하고, AWS 클라우드에 배포한 뒤, 킬체인 기반 공격 시나리오 3개를 설계·실행하는 것이 목표였다. 단순히 취약점을 나열하는 것이 아니라, 하나의 취약점이 다음 공격으로 이어지는 **체인(Chain)** 구조를 핵심으로 삼았다. 모든 과정은 교육 목적의 통제된 환경에서 수행되었다.
