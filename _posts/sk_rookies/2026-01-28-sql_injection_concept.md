...
excerpt: "SK Rookies 침투 테스트 과정에서 배운 SQL Injection의 핵심 원리와 논리적 접근 방법을 정리한 학습자료"

categories: ['sk_rookies']
typora-root-url: ../../
...
SK Rookies 침투 테스트 과정에서 배운 SQL Injection의 핵심 원리와 논리적 접근 방법을 정리한 학습자료입니다.

> "공격구문 외우고 명령어 외우는 수업이 아닙니다. 원리를 이해하고 나서 AI한테 질문해서 원하는 답을 얻어내는 게 중요합니다."
>
> "그냥 `or 1=1` 넣는 게 무슨 의미가 있습니까. 왜 이렇게 되는지 원리를 이해해야 합니다."
...
상태를 저장하지 않기 때문에 **세션(Session)과 쿠키(Cookie)** 개념이 등장합니다.
- **Session**: 서버에 저장
- **Cookie**: 브라우저에 저장
...
500 에러는 "개발자가 예외처리를 제대로 안 해놨다"는 신호입니다. SQL Injection 시 이 에러가 뜨면 쿼리가 DB까지 전달되고 있다는 증거입니다.
...
INSERT, UPDATE, DELETE에서 공격하면 실제로 DB가 수정됩니다!

**실제 사고 사례**: 업데이트 쿼리 잘못 때려서 전체 회원의 비밀번호가 '1'로 바뀜. 암호화된 값도 아니라서 어떤 비밀번호로도 로그인 불가. 백업도 없어서 몇 달간 난리.

"이거 하면 진짜 사고입니다. 책임은 저희가 집니다."
...
WHERE절은 **검색 기능이 아니라 필터링 기능**입니다!

한 줄씩 가져와서 조건이 **참/거짓**인지 판단하고, 참인 것만 결과에 포함시킵니다.

### 3.2 왜 이해가 중요한가?

SQL Injection은 이 **참/거짓 판단 로직**을 조작하는 것입니다.
...
조건을 **항상 참**으로 만들면 모든 데이터가 "합격"됩니다.
...
T OR F → T (하나라도 참이면!)
F OR T → T
F OR F → F (둘 다 거짓일 때만 거짓)
```

### 4.2 연산 순서

**AND가 OR보다 먼저 실행됩니다!** (곱하기 > 더하기)
...
"쿼리문 유추가 반드시 되어야 합니다. 유추 없이 공격하면 그냥 치트시트 복붙하는 수준밖에 안 됩니다."
...
"실무에서 주석을 날리면 뒤에서 무슨 일이 벌어질지 모릅니다."
...
`'q%'='q'`에서 뒤에 `%'`가 붙으면 `'q%'='q%'`가 됩니다.
LIKE 패턴에서 `%`는 "0개 이상의 문자"이므로 `'q%'`는 `'q'`를 포함해서 매칭됩니다.
...
"버프 스위트 없으면 해킹 못 해요?" 이건 "디자이너한테 포토샵 없으면 그림 못 그려요?" 같은 질문입니다.

버프 사용법을 아는 사용자가 되지 말고, **원리를 아는 전문가**가 되세요.
...
"남는 건 보고서밖에 없습니다. 아무리 뛰어난 해커여도 보고서를 못 쓰면 실력이 없다고 평가받습니다."
...
- EQST MS 실습 사이트: https://lab.eqst.co.kr
- 강의 노트: https://seyong.kr
- Oracle Error Help: https://docs.oracle.com/error-help/db/
